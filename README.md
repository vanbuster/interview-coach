...
**跨平台**：macOS（Metal 加速）、Linux（CPU / NVIDIA CUDA）、Windows 均可使用。引擎自动检测，无需手动选择。
...
macOS 额外安装 mlx-whisper（Metal 加速），实现本地最优转写速度。
...
| 平台 | 引擎 | 加速 | 安装方式 |
|------|------|------|---------|
| macOS Apple Silicon | mlx-whisper | Metal GPU | `setup.sh` 自动安装 |
| Linux / macOS (Intel) | faster-whisper (CTranslate2) | CPU INT8 | `setup.sh` 自动安装 |
...
| Python 3.10+ | ✅ | 系统包管理器 |
| faster-whisper | ✅ | `setup.sh` / `setup.ps1` 自动安装 |
| mlx-whisper | ❌ macOS | `setup.sh` 自动安装（Metal 加速） |
| mlx-audio | ❌ macOS | `setup.sh` 自动安装（SenseVoice） |
